...
- **📈 Portfolio-level backtest** (`/api/sim/portfolio`, `portfolio_backtest.py`, 📈
  ...
  **equity curve** plus **CAGR, max drawdown, Sharpe, profit factor, exposure**, overall
  ...
  When more signals fire than free slots, the book takes the **highest-conviction** ones:
  every trade carries an entry-time score (0-100) from its own trigger magnitude (move ×
  volume, breakout distance, OI%, gap size, relative strength…), no look-ahead. That one
...
| `GET /api/eod/conviction/calibration?days=N` | **Does stacking pay?** — scores the saved conviction ideas' realized TARGET/STOP outcomes: win rate by pillar count, per-pillar lift (measured weight, out-of-sample verdict, applied weight), option-⚠️ impact + verdict |
| `GET /api/eod/conviction?...&adaptive=1` | **⚖️ Adaptive weighting** — apply each pillar's calibration-measured edge to the board's scoring (re-orders within a confirmation tier; only where the edge held **out of sample**, else neutral — `adaptiveValidation` says why) |
| `POST /api/ideas/resolve[?days=N&force=1]` | Settle unresolved **past-day** ideas (the conviction board's, plus live ideas that outlived their session) against stored daily bars — no network. Runs automatically post-close; `force` re-scores only previous daily verdicts |
| `GET /api/eod/scheduler` · `POST /api/eod/scheduler/run` | Auto post-close EOD refresh: state (enabled/runAt/lastRun/dueToday) · trigger a backfill+deals+**idea settlement**+digest now (off-thread) |
...
├── requirements.txt
├── docker-compose.otel.yml # Local OpenTelemetry backend (grafana/otel-lgtm) — see web/observability.py
├── README.md
...
├── nse_pulse/              # application package — import as `from nse_pulse.<sub> import <mod>`
│   ├── core/
...
│   ├── eod/
│   │   ├── bhavcopy.py         # EOD UDiFF bhavcopy + sec_bhavdata_full delivery% — price/universe fallback + backfill
│   │   ├── deals.py            # Bulk/block deals (institutional footprint) from nsearchives CSV
│   │   ├── eod_scanner.py      # Full-market EOD/swing scanner over db.eod_bars — off-hours
│   │   ├── eod_conviction.py   # EOD conviction board — fuses breakout+delivery+deals+OI+RS+chain+rollover
│   │   ├── eod_options.py      # Resilient EOD option chain from FO bhavcopy (PCR/max-pain/OI walls)
│   │   ├── eod_scheduler.py    # Auto post-close EOD refresh — block-aware daemon
│   │   ├── conviction_calibration.py # Does stacking pay? per-pillar lift + verdict; adaptive pillar_weights(), OOS-gated
│   │   ├── rollover.py         # Futures rollover tracker off the FO bhavcopy (roll%/cost/basis/net-OI)
│   │   ├── sector_scan.py      # Sector relative-strength (rotation) board over db.eod_bars
...
├── tests/                  # 974 unit tests, 40 suites (import from nse_pulse.<sub>)
...
└── *.json                  # (gitignored) sim_state.json, paper_state.json, ideas_journal.json, *_config.json
```
...
**Akamai / "Access Denied" backoff.** NSE's edge (Akamai) blocks an IP with an
HTTP 403 *"Access Denied … edgesuite.net"* page once it looks bot-like — usually
