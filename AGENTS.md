...
├── requirements.txt
├── docker-compose.otel.yml  # local OTel backend (grafana/otel-lgtm) for web/observability.py — dev only
├── README.md
├── AGENTS.md          # <- this file
├── nse_pulse/         # the app package — every module imports as `from nse_pulse.<sub> import <mod>`
...
│   ├── sim/           # forward-testing + paper trading
...
│   │   ├── eod_scanner.py     # Full-market EOD/swing scanner over db.eod_bars (breakouts/gaps/vol/MA/NR7/delivery + xref)
...
│   │   ├── backtest_daily.py      # Daily-bar historical backtest, 9 strategies — source live (curated) or eod (whole universe)
...
│   ├── web/           # HTTP layer
│   │   ├── app.py             # Flask server + JSON API endpoints + main() (runs on port 5055)
│   │   ├── observability.py   # Per-request access log (token-redacted) + opt-in OpenTelemetry (OTLP); lazy, no-ops if absent
│   │   ├── notify.py          # Off-screen alerts (Telegram/webhook), opt-in; EOD digest track-record footer
...
├── .gitignore
├── sim_state.json / paper_state.json / ideas_journal.json  # (gitignored) local state, at the repo root
...
### Package layout & imports (post-restructure)

- Code lives in the **`nse_pulse/`** package, grouped by domain (`core` / `feeds` /
  `sim` / `eod` / `backtest` / `web` / `cli`). Every intra-project import is absolute:
  `from nse_pulse.core import nse_client as nse`, `from nse_pulse.eod import bhavcopy`,
  etc. There are **no** top-level flat modules anymore.
- **Resource files stay at the repo root**, resolved via `nse_pulse/core/paths.py`
  (`paths.PROJECT_ROOT` / `paths.DATA_DIR` / `paths.root(...)`): `data/market.db`, the
  `*_config.json`, `sim_state.json` / `paper_state.json` / `ideas_journal.json`, and
  `logs/`. Never re-derive these from `os.path.dirname(__file__)`.
- **Entry points are thin root shims** (`app.py`, `nse_demand.py`) that import and call
  the package `main()`. `python app.py` / `python nse_demand.py` work unchanged; the
  Flask reloader re-execs the root shim.
- **Tests** (`tests/`) import the same package paths. `pyproject.toml` sets
  `pythonpath=["."]` so `import nse_pulse...` resolves without an editable install.
...
- **Ideas + manual Sim Take gated to market hours** — `get_recommendations()` (the 💡 Ideas tab + the desktop-notify
  ...
  `nse._is_market_open()`; `POST /api/sim/take` takes nothing outside 09:15–15:30 IST. Auto-take was already loop-gated;
  historical/analysis views (summary/daily/leaderboard/performance/analytics, strategy-of-day, EOD board) stay live. Suite 846 → 848.
...
- **Fix: `/api/health` never blocks on the pacer lock** — `_pace()` used to hold `_pace_lock` across its
  ...
  during background NSE bursts. Now it reserves the start slot under the lock and sleeps after releasing (same
  spacing; workers also sleep concurrently now). Suite 844 → 845.
...
- **Restructure: flat root → `nse_pulse/` package** — moved all 30 modules into
  ...
  added `nse_pulse/core/paths.py` so `data/`, the `*_config.json`, state JSON and
  `logs/` still live at the repo root. `app.py` / `nse_demand.py` are now thin root
  shims → package `main()`; tests moved to `tests/`, audits to `docs/`; added
  `pyproject.toml` (pytest `pythonpath`/`testpaths`). `python app.py` / `python
  nse_demand.py` unchanged; suite still 826 green + both entry points smoke-verified.
...
- **Per-endpoint NSE request budget** — the pacer knew the total rate but not WHERE it went.
  ...
  under `/api/health.nse.endpoints`, so the next trim targets the heaviest callers with data.
  Bucketed by path (query dropped) → ~15-20 stable endpoints. Tests **+5** (25 → 30) + health
  assert; suite **791 → 796**.
...
- **Trim NSE load at the source** — complements the pacer with fewer *total* market-hours
  ...
  cadences also env-tunable; `STALE_AFTER = max(180, INTERVAL*2)`), and `build_context`'s
  per-symbol quote+candle fan-out **45 → 30** candidates (env `NSE_CTX_CANDIDATES`,
  `strategies._CTX_CAND`, floor 10). ≈55% less per-symbol NSE volume; trade-off is 90s
  snapshot granularity + 30 (vs 45) intraday candidates, both dial-able. Tests **+4**; suite
  **772 → 776**.
- **Global NSE request pacer + escalating cooldown + browser headers** — user kept hitting
  the **NSE Akamai** block. An audit showed every NSE hit funnels through the one warmed
  `requests.Session`, and nothing smoothed the **bursts** from the 6-8 worker fan-outs (cold
  `snapshot_logger`/`build_context()`) — the per-IP burst the WAF flags. `_build_session()`
  now returns a **`_PacedSession`** whose `send()` paces ALL NSE traffic at one choke point:
  ≤`_NSE_MAX_CONCURRENCY=4` in flight, starts `_NSE_MIN_GAP=0.20s`(+jitter) apart, soft
  `_NSE_SOFT_RPM=120`/min ceiling (`nse_quote`/`bhavcopy` inherit it, no changes). `note_block`
  now **escalates** the cooldown on repeat blocks (600→1200→2400…, cap `_BLOCK_MAX=3600`,
  resets after a clean gap). `HEADERS` gained modern-Chrome client hints + `Sec-Fetch-*`
  (warm-ups use navigation `_NAV_HEADERS`). `/api/health.nse` = `pacer_stats()` (blockCount/
  reqLastMin/…); the banner notes repeat blocks. Pure-Python (curl_cffi TLS swap deferred to
  Phase 2). Tests **+12** (`test_nse_client.py`); suite **760 → 772**.
- **Short TTL cache for broker candles** — re-opening the same stock/interval (or the modal's
  rest_ohlc + rest_chart fallback + the Live seed wanting the same series) was re-hitting
  Angel's rate-limited getCandleData. Added `angel_feed._candle_cache` (keyed by token,
  interval, from-DATE; **30s TTL**; 256-entry cap) wired into `_get_candles` with
  double-checked locking — cache hits serve without the lock; only successes cached; failures
  retry. Fewer Angel calls, snappier UI, more headroom under 180/min. Tests **+3**; suite
  **757 → 760**.
...
- Code is a package: `nse_pulse/{core,feeds,sim,eod,backtest,web,cli}`. Import
  absolutely (`from nse_pulse.<sub> import <mod>`); resolve repo-root files via
  `nse_pulse/core/paths.py`, never `os.path.dirname(__file__)`.
- Keep data-fetching logic in `core/nse_client.py` / `core/nse_quote.py` / the
  live-feed adapters (`feeds/angel_feed.py` / `feeds/dhan_feed.py`); keep
  `web/app.py` thin (routes only).
